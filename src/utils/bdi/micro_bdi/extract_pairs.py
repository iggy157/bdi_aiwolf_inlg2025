...
DEFAULT_OUT_SUBDIR = "talk_history"
...
def load_talk_file_with_headers(p: Path) -> tuple[list[dict], dict[str, Any]]:
    """Load talk history file and return items and headers separately.
    
    Returns:
        tuple: (items_list, headers_dict)
    """
    if not p.exists():
        return [], {}
    ...
    if isinstance(data, dict):
        ...
        if isinstance(items, list):
            headers = {k: v for k, v in data.items() if k != "items"}
            return [x for x in items if isinstance(x, dict)], headers
    
    return [], {}


def find_talk_dir(agent_dir: Path, candidates: tuple[str, ...]) -> Optional[Path]:
...
def extract_pairs_for_agent(
    ...
    *,
    ...
    talk_dir_candidates: tuple[str, ...] = ("talk_history", "トーク履歴"),
) -> dict[str, list[dict]]:
    ...
    if flat_output:
        # Use talk_dir directly: <agent_dir>/<talk_dir>/<from>.yml
        talk_dir = find_talk_dir(agent_dir, talk_dir_candidates)
        if not talk_dir:
            # Create default talk directory
            talk_dir = agent_dir / talk_dir_candidates[0]
            talk_dir.mkdir(parents=True, exist_ok=True)
        out_base = talk_dir
    else:
        # Traditional mode: <agent_dir>/<out_subdir>/<from>.yml
    ...
    for from_name, items in grouped.items():
        ...
        try:
            if append and flat_output:
                # Load with headers for flat output
                existing_items, headers = load_talk_file_with_headers(out_path)
                
                # Preserve existing headers, set defaults if missing
                if "liking" not in headers:
                    headers["liking"] = 0.5
                if "creditability" not in headers:
                    headers["creditability"] = 0.5
                
                # Merge items
                merged_items = dedup_append(existing_items, items)
                
                # Write with headers
                with out_path.open("w", encoding="utf-8") as f:
                    f.write("# generated by utils.bdi.micro_bdi.extract_pairs\n")
                    # Write headers first
                    output_data = dict(headers)
                    output_data["items"] = merged_items
                    yaml.safe_dump(output_data, f, sort_keys=False, allow_unicode=True, default_flow_style=False)
                logger.info("Saved %d items with headers -> %s", len(merged_items), out_path)
            elif append:
                # Traditional append mode
                ...
                # Overwrite mode
                if flat_output:
                    # Create minimal structure with headers
                    output_data = {
                        "liking": 0.5,
                        "creditability": 0.5,
                        "items": items
                    ...
                    with out_path.open("w", encoding="utf-8") as f:
                        f.write("# generated by utils.bdi.micro_bdi.extract_pairs\n")
                        yaml.safe_dump(output_data, f, sort_keys=False, allow_unicode=True, default_flow_style=False)
                    logger.info("Created %d items with headers -> %s", len(items), out_path)
                else:
    ...
    for a in agents:
        try:
            results[a] = extract_pairs_for_agent(
                base_dir, game_id, a, out_subdir, skip_pending,
                flat_output=flat_output, exclude_self=exclude_self, append=append,
                talk_dir_candidates=talk_dir_candidates
            )
        except Exception as e:
...
def main() -> int:
    ...
    p.add_argument("--subdir-name", default=DEFAULT_OUT_SUBDIR, help="Output subdir (default: talk_history)")
    ...
    p.add_argument("--no-flat-output", action="store_true", help="Use traditional output structure")
    ...
    try:
        res = extract_pairs_for_game(
            ...
            flat_output=(not args.no_flat_output),
